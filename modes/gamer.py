...
class GamerMode(BaseMode):
    """Modo Gamer com integração RetroArch"""
    ...
    async def enter(self):
        ...
        self.print_mode_header("MODO GAMER / RETROARCH")
        
        print(f"{Fore.GREEN}Yatta! Modo gamer ativado! 🎮{Style.RESET_ALL}\n")
        
        # Verifica RetroArch
        if not self.retroarch_path:
            print(f"{Fore.YELLOW}⚠️ RetroArch não encontrado!{Style.RESET_ALL}")
            print(f"{Fore.CYAN}Baixe em: https://www.retroarch.com{Style.RESET_ALL}")
            print(f"{Fore.CYAN}Instale em C:/RetroArch/{Style.RESET_ALL}\n")
            
            self.speaker.speak("RetroArch não tá instalado! Precisa baixar primeiro, ne~")
        else:
            print(f"{Fore.GREEN}✓ RetroArch encontrado: {self.retroarch_path}{Style.RESET_ALL}\n")
    ...
    async def show_gamer_menu(self):
        ...
        while self.is_active:
            ...
            print("1. 🎮 Jogar (RetroArch)")
            print("2. 💬 Chat da Live")
    ...
    async def play_retro_games(self):
        ...
        print(f"{Fore.MAGENTA}🎮 JOGOS DISPONÍVEIS{Style.RESET_ALL}")
    ...
    async def launch_game(self, game):
    ...
    async def process_chat_message(self, message):
        ...
        # Usa IA para responder naturalmente
